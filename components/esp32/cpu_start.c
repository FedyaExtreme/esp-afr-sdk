...
// If CONFIG_SPIRAM_IGNORE_NOTFOUND is set and external RAM is not found or
// errors out on testing, this is set to false.
static bool s_spiram_okay = true;

/*
 * We arrive here after the bootloader finished loading the program from flash.
 * The hardware is mostly uninitialized, and the app CPU is in reset. We do have
 * a stack, so we can do the initialization in C.
 */
...
void IRAM_ATTR call_start_cpu0(void) {
  ...
  // Clear BSS. Please do not attempt to do any complex stuff (like early
  // logging) before this.
  memset(&_bss_start, 0, (&_bss_end - &_bss_start) * sizeof(_bss_start));

#ifdef CONFIG_ESP32_IRAM_AS_8BIT_ACCESSIBLE_MEMORY
  // Clear IRAM BSS
  memset(&_iram_bss_start, 0,
         (&_iram_bss_end - &_iram_bss_start) * sizeof(_iram_bss_start));
#endif

  /* Unless waking from deep sleep (implying RTC memory is intact), clear RTC
   * bss */
...
#if CONFIG_SPIRAM_ALLOW_BSS_SEG_EXTERNAL_MEMORY
    ESP_EARLY_LOGE(
        TAG, "Failed to init external RAM, needed for external .bss segment");
    abort();
#endif
  ...
  if (LOG_LOCAL_LEVEL >= ESP_LOG_INFO) {
    ...
    ESP_EARLY_LOGI(TAG, "Compile time:     %s %s", app_desc->date,
                   app_desc->time);
#endif
    char buf[17];
  ...
  if (REG_GET_BIT(EFUSE_BLK0_RDATA3_REG, EFUSE_RD_CHIP_VER_DIS_APP_CPU)) {
    ESP_EARLY_LOGE(TAG, "Running on single core chip, but application is built "
                        "with dual core support.");
    ESP_EARLY_LOGE(
        TAG, "Please enable CONFIG_FREERTOS_UNICORE option in menuconfig.");
    abort();
  }
  ...
  memset(&_ext_ram_bss_start, 0,
         (&_ext_ram_bss_end - &_ext_ram_bss_start) *
             sizeof(_ext_ram_bss_start));
#endif
  /* Initialize heap allocator. WARNING: This *needs* to happen *after* the app
     cpu has booted. If the heap allocator is initialized first, it will put
     free memory linked list items into memory also used by the ROM. Starting
     the app cpu will let its ROM initialize that memory, corrupting those
     linked lists. Initializing the allocator *after* the app cpu has booted
     works around this problem.
     With SPI RAM enabled, there's a second reason: half of the SPI RAM will be
     managed by the app CPU, and when that is not up yet, the memory will be
     inaccessible and heap_caps_init may fail initializing it properly. */
  heap_caps_init();
...
static void wdt_reset_cpu1_info_enable(void) {
  DPORT_REG_SET_BIT(DPORT_APP_CPU_RECORD_CTRL_REG,
                    DPORT_APP_CPU_PDEBUG_ENABLE | DPORT_APP_CPU_RECORD_ENABLE);
...
// Enable trace memory and immediately start trace.
  ...
  CLEAR_PERI_REG_MASK(UART_CONF0_REG(CONFIG_ESP_CONSOLE_UART_NUM),
                      UART_TICK_REF_ALWAYS_ON);
#else
  const int uart_clk_freq = APB_CLK_FREQ;
#endif // CONFIG_PM_DFS_ENABLE
  uart_div_modify(CONFIG_ESP_CONSOLE_UART_NUM,
                  (uart_clk_freq << 4) / CONFIG_ESP_CONSOLE_UART_BAUDRATE);
  ...
  const char *default_uart_dev =
      "/dev/uart/" STRINGIFY(CONFIG_ESP_CONSOLE_UART_NUM);
  _GLOBAL_REENT->_stdin = fopen(default_uart_dev, "r");
  _GLOBAL_REENT->_stdout = fopen(default_uart_dev, "w");
  _GLOBAL_REENT->_stderr = fopen(default_uart_dev, "w");
#else  // defined(CONFIG_VFS_SUPPORT_IO) &&
       // !defined(CONFIG_ESP_CONSOLE_UART_NONE)
  _REENT_SMALL_CHECK_INIT(_GLOBAL_REENT);
#endif // defined(CONFIG_VFS_SUPPORT_IO) &&
       // !defined(CONFIG_ESP_CONSOLE_UART_NONE)
       // After setting _GLOBAL_REENT, ESP_LOGIx can be used instead of
       // ESP_EARLY_LOGx.
  ...
  assert(!soc_has_cache_lock_bug() &&
         "ESP32 Rev 3 + Dual Core + PSRAM requires INT WDT enabled in project "
         "config!");
#endif
#endif
...
#if !CONFIG_SPIRAM_BOOT_INIT
  // Read the application binary image header. This will also decrypt the header
...
#else
  // This assumes that DROM is the first segment in the application binary, i.e.
  // that we can read the binary header through cache by accessing SOC_DROM_LOW
  // address.
  ...
  portBASE_TYPE res =
      xTaskCreatePinnedToCore(&main_task, "main", ESP_TASK_MAIN_STACK, NULL,
...
void start_cpu1_default(void) {
  ...
  esp_int_wdt_cpu_init();
#endif
  // Take care putting stuff here: if asked, FreeRTOS will happily tell you the
  // scheduler has started, but it isn't active *on this CPU* yet.
  esp_cache_err_int_init();
  esp_crosscore_int_init();
  esp_dport_access_int_init();
...
static void main_task(void *args) {
#if !CONFIG_FREERTOS_UNICORE
  // Wait for FreeRTOS initialization to finish on APP CPU, before replacing its
  // startup stack
  ...
  const esp_partition_t *efuse_partition = esp_partition_find_first(
      ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_EFUSE_EM, NULL);
  if (efuse_partition) {
